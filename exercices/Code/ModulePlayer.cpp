...
void ModulePlayer::updateState() 	{
	...
	// TODO 2
	// Check if the current state is movement. If it is, update the current state with the wanted state and update 
	// the current animation depending on the current state (Update_animation_depending_on_current_state()).

	// If it is not movement, check if the current state can be cancelled into the wanted one
	// (Can_cancel_current_state_into(wanted_state)) and if the current frame is an active one
	// (current_animation->GetState() == ACTIVE). 

		// If you can do the cancel, update the current state, reset the current animation and update
		// animation depending on the current state.

		// If you can't, check if the animation has finished, and make the current state be IDLE
		// or CROUCHING, depending on what the player is pressing (direction_inputs.down).
		// then reset the current animation and update it.
}
...
{
	// TODO 3 (optional)
	// Check if there is a tatsumaki in the input buffer. 

	// The combination is stored in the "tatsumaki_inputs" list.

	// Iterate all the array and check if you can find the same combination. Remember to use the
	// "cancelability window".

	// Return "true" if it is found, and "false" if it is not.

	// Uncomment this function in requestState()
	...
	{
			input_iterator++; // Use this to go to the next element in the list
	}

	if (false /*Insert condition to return true*/)
		return true;
	else
		return false;
}
...
{
	// TODO 1
	// Introduce the argument of this function into the "input_buffer". Introduce it as the last
	// element and shift the others to the right (element 1 will be 0, element 0 will be erased)
}
